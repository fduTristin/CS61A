...
def missing_digits(n):
    ...
    def count(num, last):
        if num == 0:
            return 0
        if num % 10 != last:
            return (last - num % 10 - 1) + count(num // 10, num % 10)
        else:
            return count(num // 10, last)
...
def count_change(total):
    ...
    def cnt_chg(rest, max):
        if rest == 0:
            ...
            return cnt_chg(rest - max, max) + cnt_chg(rest, max // 2)

    def find_pow2(num):
        if num == 0:
            return 0
        else:
            return 1 + find_pow2(num // 2)

    return cnt_chg(total, 2 ** (find_pow2(total) - 1))
